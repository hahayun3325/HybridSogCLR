...
def train(model, data_loader, optimizer, tokenizer, epoch, max_epoch, warmup_steps, device, scheduler, grad_scaler,
    ...
    for meter in metric_logger.meters.values():
        meter.update(0.0)  # Initialize with zero

    # Add hybrid-specific metrics if needed
    if args.ita_type == 'hybrid':
        metric_logger.add_meter('strategy_weights', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
        metric_logger.add_meter('memory_bank_size', utils.SmoothedValue(window_size=1, fmt='{value:.0f}'))
        # Initialize these metrics as well
        metric_logger.meters['strategy_weights'].update(0.0)
        metric_logger.meters['memory_bank_size'].update(0.0)
    ...
    for i, (image, text, idx, text_idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if grad_scaler is None:
            ...
            with torch.cuda.amp.autocast():
                loss_ita, info_dict = model(image, text_input, idx=idx, text_idx=text_idx, epoch=epoch,
                                            max_epoch=max_epoch)
...
def verify_paths(args):
    ...
    for name, path in required_paths.items():
        if not os.path.exists(path):
            ...
            print(f"Absolute path attempted: {os.path.abspath(path)}")
...
def main(args):
    ...
    print(f"train_image_root: {args.train_image_root}")
    print(f"data_path: {args.data_path}")
    print(f"ann_path: {args.ann_path}")
    ...
    #### Zero-shot transfer ####
    if args.zs_dataset:
        zeroshot_dataloader = create_zeroshot_dataloader(dataset_name=args.zs_dataset, data_folder=args.zs_datafolder,
    ...
    model = CLIP(
        ...
        rho_I=args.rho_I,
        rho_T=args.rho_T,
        rho_init=args.rho,  # Added for iSogCLR variants
        ...
        # New hybrid-specific parameters (if using hybrid loss)
        num_strategies=args.num_strategies if args.ita_type == 'hybrid' else 3,
        neg_samples_per_strategy=args.neg_samples_per_strategy if args.ita_type == 'hybrid' else 256,
        memory_bank_size=args.memory_bank_size if args.ita_type == 'hybrid' else 65536,
        memory_bank_momentum=args.memory_bank_momentum if args.ita_type == 'hybrid' else 0.99,
        hybrid_tau_min=args.hybrid_tau_min if args.ita_type == 'hybrid' else 0.005,
        hybrid_tau_max=args.hybrid_tau_max if args.ita_type == 'hybrid' else 1.0,
    )
    ...
    if args.check_samples_tau:
        ...
        with torch.no_grad():
            for image, text, idx, text_idx in tqdm(train_loader):
                image = image.to(device)
                text = tokenizer(text, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(
                    device)
    ...
    for epoch in range(0, max_epoch):
        if not args.evaluate:
            ...
            train_stats = train(model, train_loader, optimizer, tokenizer, epoch, max_epoch, warmup_steps, device,
                                lr_scheduler,
                                grad_scaler, args)

        if args.evaluate:
            score_val_i2t_coco, score_val_t2i_coco = evaluation(model_without_ddp, val_coco_loader, tokenizer, device,
                                                                args)
        ...
        if utils.is_main_process():
            ...
            if args.evaluate:
                val_result_coco = itm_eval(score_val_i2t_coco, score_val_t2i_coco, val_coco_loader.dataset.txt2img,
                ...
                if args.zs_dataset:
                    zeroshot_results = zeroshot_transfer(model_without_ddp, zeroshot_dataloader, args.zs_dataset,
                                                         tokenizer, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ita_type', required=True, choices=['clip', 'cyclip', 'vicreg', 'sogclr', 'sogclr_dro', 
                        'isogclr_new_v2', 'isogclr_new_v1', 'isogclr_new', 'hybrid','onlineclr', 'sogclr_mine'])
    ...
    # Add rho_init for iSogCLR variants
    parser.add_argument('--rho', default=6.0, type=float,
                       help='Initial rho value for iSogCLR variants')
    ...
    # added loss config for the hybrid method
